...
da_meta <- read_tsv("data/processed/final_data/final_data_all_batches.tsv") 
...
patients_exclude <- 
  ukb_meta_ext |> 
  filter(!Group %in% c("Healthy", groups))

patients_de_ukb <- 
  ukb_meta |> 
  filter(!is.na(Sex),
         !is.na(Age),
         !id %in% patients_exclude$id) |> 
  select(DAid = id, Sex, Age, Disease) |> 
  mutate(BMI = 0)

wide_data_ukb <- 
  ukb_data |>
  filter(id %in% patients_de_ukb$DAid) |> 
  select(DAid = id, Assay, NPX) |> 
  pivot_wider(names_from = Assay, values_from = NPX) 

patients_de_ukb <- 
  patients_de_ukb |> 
  filter(DAid %in% wide_data_ukb$DAid) |> 
  distinct()

# Run differential expression analysis
de_results_ukb <- 
  de_limma_disease(data_wide = wide_data_ukb, 
                   metadata = patients_de_ukb,
...
de_ukb |> 
  ...
  geom_text_repel(data = labels_ukb, size = 2, show.legend = F) +
...
ggsave(savepath("volcano_ukb_3_5yearsbefore.png"), h = 5, w = 5)

de_results_ukb <- 
  de_results_ukb |> 
  mutate(Assay = sub("\\.{3}\\d+$", "", Assay)) |> 
  mutate(sig = ifelse(adj.P.Val < 0.05 & logFC < 0, "significant down",
                    ifelse(adj.P.Val < 0.05 & logFC > 0, "significant up", "not significant")))
```
...
patients_exclude <- 
  ukb_meta_ext |> 
  filter(!Group %in% c("Healthy", group))
...
patients_de_cptac <- 
  cptac_data_paired |> 
  distinct(Sample) |> 
  mutate(Patient_ID = gsub("\\.N$", "", Sample),
         Disease = ifelse(str_detect(Sample, "\\.N$"), "Healthy", "Pancreatic cancer"))  |> 
  left_join(cptac_meta |> 
              select(Patient_ID, Age = age, Sex = sex), by = "Patient_ID") |> 
  filter(!is.na(Sex), 
         !is.na(Age)) |> 
  mutate(BMI = "") |> 
  select(DAid = Sample, Disease, Age, Sex, BMI)


wide_data_cptac <- 
  cptac_data_paired |> 
  rename(DAid = Sample) |> 
  filter(DAid %in% patients_de_cptac$DAid) |> 
  mutate_at(c(2:ncol(cptac_data_paired)), as.numeric)

exclude <- 
  wide_data_cptac |> 
  pivot_longer(names_to = "Assay", values_to = "exp", cols = c(2:12018)) |> 
  filter(!is.na(exp)) |> 
  group_by(Assay) |> 
...
wide_data_cptac_complete <- 
  wide_data_cptac |> 
  select(all_of(c("DAid", include)))

de_results_cptac <- 
  de_limma_disease(data_wide = wide_data_cptac_complete, 
                   metadata = patients_de_cptac,
                   disease = "Pancreatic cancer",
                   correct = T) 
...
de_cptac <- 
  de_results_cptac |> 
  mutate(Assay = sub("\\.{3}\\d+$", "", Assay)) |> 
  mutate(sig = ifelse(adj.P.Val < 0.05 & logFC < -1, "significant down",
                    ifelse(adj.P.Val < 0.05 & logFC > 1, "significant up", "not significant")))


labels_cptac <- 
  de_cptac |> 
  filter(Assay %in% unique(cancer_data_long$Assay)) |> 
  top_n(n = 10, wt = -log10(adj.P.Val)) 
...
de_cptac |> 
  ...
  ggplot(aes(x = logFC, y = -log10(adj.P.Val), color = sig, label = Assay)) +
  geom_point(size = 1, alpha = 0.4, show.legend = F) + 
  geom_text_repel(data = labels_cptac, size = 2, show.legend = F) +
  geom_hline(yintercept = -log10(0.05), linetype = 'dashed', color = "darkgrey") +
  geom_vline(xintercept = -1, linetype = 'dashed', color = "darkgrey") +
  geom_vline(xintercept = 1, linetype = 'dashed', color = "darkgrey") +
  scale_color_manual(values = pal_de) +
...
ggsave(savepath("cptac_volcano.png"))
...
ggsave(savepath("cohorts_da.pdf"),
       height = 4,
       width = 7)
```
...
scores + loadings
ggsave(savepath("pca_pancreas.pdf"))

scores
ggsave(savepath("PCA_scores.pdf"), h =5, w=5)

library("FactoMineR")
library(factoextra)
dat <-  cancer_data_wide |> 
  filter(Disease %in% c("PAN", "CAD healthy")) |> 
  select(-Disease) |> 
  impute_values(ID = "DAid", wide_data = T) |> 
  column_to_rownames("DAid") |> 
